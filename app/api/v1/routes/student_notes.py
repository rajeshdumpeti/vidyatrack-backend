...
def _resolve_author_meta(db: Session, school_id: int, user_ids: set[int]):
    ...
    if teacher_user_ids:
        teachers_by_user = {
            ...
            .filter(
                ...
                Teacher.user_id.in_(teacher_user_ids),
            )
            .all()
        }
    ...
    if principal_user_ids:
        principals_by_user = {
            p.user_id: p
            for p in db.query(Principal)
            .filter(
                Principal.school_id == school_id,
                Principal.user_id.in_(principal_user_ids),
            )
            .all()
        }
    ...
    for user_id, user in users_by_id.items():
        role = user.role
        if role == "TEACHER":
            name = teachers_by_user.get(user_id).name if teachers_by_user.get(
                user_id) else "Teacher"
        elif role == "PRINCIPAL":
            name = principals_by_user.get(user_id).name if principals_by_user.get(
                user_id) else "Principal"
...
def create_student_note(
    ...
    meta = _resolve_author_meta(
        db, current_user["school_id"], {current_user["user_id"]})
    author = meta.get(current_user["user_id"], {})
...
def list_student_notes(
    ...
    meta = _resolve_author_meta(
        db, current_user["school_id"], author_ids)
