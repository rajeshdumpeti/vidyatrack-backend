...
def upsert_management_principal_internal(
    *,
    ...
    existing_user = db.query(User).filter(User.phone.in_(phone_candidates(normalized_phone))).first()
...
def _consume_valid_otp(db: Session, phone: str, otp: str, *, otp_pepper: str) -> None:
    now = datetime.now(timezone.utc)
    ...
    provided_hash = _hash_otp(normalized_phone, otp, otp_pepper=otp_pepper)
...
def verify_principal_onboarding(
    *,
    ...
    _consume_valid_otp(db, session.phone, otp.strip(), otp_pepper=otp_pepper)

    principal = upsert_management_principal_internal(
        payload=ManagementPrincipalIn(
            school_id=school_id,
            name=session.name,
            phone=session.phone,
...
def upsert_management_principal(
    *,
    ...
    return upsert_management_principal_internal(
        payload=payload,
        response=response,
        db=db,
        current_user=current_user,
...
def register_management_principal_with_otp(
    *,
    ...
    try:
        otp_result = request_otp_fn(otp_request_payload_factory(phone=principal.phone), request, db)
